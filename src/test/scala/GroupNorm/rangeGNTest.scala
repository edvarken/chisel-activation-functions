...
class rangeGNTest extends AnyFreeSpec with Matchers {
    "rangeGNTest should correctly apply range GroupNorm to every element of the input vector of C/32 elements" in {
        ...
        simulate(new rangeGN(C = 320)) { dut => 
            ...
            dut.clock.step(latencyNis10) // total latency for N=10 is 31cc
            ...
            if (verbose > 0) {
                println(s"expected-actual= ${diff_outputs}")
        ...
        simulate(new rangeGN(C = 1280)) { dut => 
            ...
            dut.clock.step(latencyNis40) // total latency for N=40 is 35 according to formula: ceil(log2(40))*3+17, it should only be 35 cc's
