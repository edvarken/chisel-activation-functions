...
class siluPWL28SegmentsTest extends AnyFreeSpec with Matchers {
    ...
    // ##############################        SiLU         ######################################################
    // #########################################################################################################
    "siluPWL28SegmentsTest should correctly apply an approximate SiLU value for a BF16 input, using 28 PWL SiLU non-uniform segments with shared intercepts for pos. and neg. segments" in {
        simulate(new siluPWL28Segments) { c =>
            ...
            c.clock.step(6) 
            c.io.out_a.expect("b0_01111111_0011110".U(16.W)) // 1.22636171429 
            ...
            mse /= (N+1).toFloat
            mse_MAE /= (N+1).toFloat
            println(f"SiLU 28 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Mean Squared Error (MSE): ${mse}")
            println(f"SiLU 28 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Mean Absolute Error (MAE): ${mse_MAE}")
            println(f"SiLU 28 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Maximum Absolute Error (Max AE): ${max_AE}")
            println("==============================")
        }
    }
}