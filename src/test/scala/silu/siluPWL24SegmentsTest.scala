...
class siluPWL24SegmentsTest extends AnyFreeSpec with Matchers {
    ...
    // ##############################        SiLU         ######################################################
    // #########################################################################################################
    "siluPWL24SegmentsTest should correctly apply an approximate SiLU value for a BF16 input, using 24 PWL SiLU uniform segments with shared intercepts for pos. and neg. segments" in {
        simulate(new siluPWL24Segments) { c =>
            ...
            mse /= (N+1).toFloat
            mse_MAE /= (N+1).toFloat
            println(f"SiLU 24 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Mean Squared Error (MSE): ${mse}")
            println(f"SiLU 24 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Mean Absolute Error (MAE): ${mse_MAE}")
            println(f"SiLU 24 PWL uniform segments with shared intercepts so 36 coefficients instead of 48: Maximum Absolute Error (Max AE): ${max_AE}")
            println("==============================")
        }
    }
}