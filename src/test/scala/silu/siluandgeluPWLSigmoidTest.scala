...
class siluandgeluPWLSigmoidTest extends AnyFreeSpec with Matchers {
    ...
    "siluandgeluPWLSigmoidTest should correctly apply an approximate SiLU value (in_select=0) for a BF16 input, using 20 PWL Sigmoid segments" in {
        simulate(new siluandgeluPWLSigmoid20Segments) { c =>
            var tolerance = 0.015625f*4
            var verbose = 0
            var step = 0.0f // declare step variable
            var a = 0.0f // declare running variable
    ...
    "siluandgeluPWLSigmoidTest should correctly apply an approximate GELU value (in_select=1) for a BF16 input, using 20 PWL Sigmoid segments" in {
        simulate(new siluandgeluPWLSigmoid20Segments) { c =>
            var tolerance = 0.015625f*4
            var verbose = 0
            var step = 0.0f // declare step variable
    ...
    "siluandgeluPWLSigmoidTest should correctly apply an approximate GELU value (in_select=1) for a BF16 input, using 20 PWL Sigmoid segments with Extra Intercepts" in {
        simulate(new siluandgeluPWLSigmoid20SegmentsExtraIntercepts) { c =>
            ...
            c.io.in_select.poke("b1".U(1.W))
            c.clock.step(8) 
            c.io.out_a.expect("b1011110111100001".U(16.W)) // -0.100210801903287099 ~ -0.10986328125
