...
/**
  ...
  * SiLU is approximated using 24 segments each linearly interpolated as y = m*x + q, using the local slope m and y-intercept q.
  * 24 segments: equally spaced x-values between -6 and 6
  ...
  */
class siluPWL24Segments extends Module {
    ...
    when (a(14,0) === "b00000000_0000000".U ) { // a = +-0
        ...
        when (in_a_fp >= "b011_00".U) { // a <= -3.0:
            when (in_a_fp >= "b100_10".U) { // a <= -4.5:
                when (in_a_fp >= "b101_10".U) { // 6.0 < a <= -5.5: index 1&24
                    when (sign === 1.U) {
                    ...
                    when (in_a_fp >= "b101_00".U) { // -5.5 < a <= -5.0: index 2&23
                        when (sign === 1.U) {
                    ...
                    }.otherwise { // -5.0 < a <= -4.5: index 3&22
                        when (sign === 1.U) {
                ...
                when (in_a_fp >= "b100_00".U) { // -4.5 < a <= -4.0: index 4&21
                    when (sign === 1.U) {
                    ...
                    when (in_a_fp >= "b011_10".U) { // -4.0 < a <= -3.5: index 5&20
                        when (sign === 1.U) {
                    ...
                    }.otherwise { // -3.5 < a <= -3.0: index 6&19
                        when (sign === 1.U) {
            ...
            when (in_a_fp >= "b001_10".U) { // a <= -1.5
                when (in_a_fp >= "b010_10".U) { // -3 < a <= -2.5: index 7&18
                    when (sign === 1.U) {
                    ...
                    when (in_a_fp >= "b010_00".U) { // -2.5 < a <= -2.0: index 8&17
                        when (sign === 1.U) {
                    ...
                    }.otherwise { // -2.0 < a <= -1.5: index 9&16
                        when (sign === 1.U) {
                ...
                when (in_a_fp >= "b001_00".U) { // -1.5 < a <= -1.0: index 10&15
                    when (sign === 1.U) {
                    ...
                    when (in_a_fp >= "b000_10".U) { // -1.0 < a <= -0.5: index 11&14
                        when (sign === 1.U) {
                    ...
                    }.otherwise { // -0.5 < a <= 0.0: index 12&13
                        when (sign === 1.U) {
 ...
 */
object siluPWL24SegmentsMain extends App {
    ChiselStage.emitSystemVerilogFile(
        new siluPWL24Segments,
