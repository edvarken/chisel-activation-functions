...
object FloatUtils {
    ...
    def floatToBigIntBF16(x: Float): BigInt = {
        val raw = java.lang.Float.floatToRawIntBits(x)
        val bf16 = raw >>> 16 // Keep upper 16 bits
        BigInt(bf16)
    }
    ...
    def doubleAdd(a: Double, b: Double): Double = {
        ...
        val (bigger_mant, smaller_mant, exp, bigger_sign) = if (aexp > bexp) {
        ...
        val (actualsign, abssum) = if (sum < 0) {
            (~bigger_sign & 1, -sum) // Use the opposite of the bigger number's sign
        } else if (sub && bigger_mant < smaller_mant) {
            (~bigger_sign & 1, sum) // Correct sign when subtraction flips the result
        } else {
            (bigger_sign, sum)
        }
        ...
        val rawres = (actualsign << 63) | ((actualexp & 0x7ffL) << 52) |
            (actualsum & ((1L << 52) - 1L))
