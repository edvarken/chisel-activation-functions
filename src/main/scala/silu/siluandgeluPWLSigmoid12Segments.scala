...
/**
  ...
  */
class siluandgeluPWLSigmoid12Segments extends Module {
    ...
    }.elsewhen (actual_exp >= 3.S) { // sigmoidInput <= -8 or sigmoidInput >= +8
        when (sign === 1.U) { // sigmoidInput <= -8
            fullRangeSigmoidReg := 0.U // sigmoid(sigmoidInput) = 0
        }.otherwise { // sigmoidInput >= +8
...
object siluandgeluPWLSigmoid12SegmentsMain extends App {
    ChiselStage.emitSystemVerilogFile(
        new siluandgeluPWLSigmoid12Segments,
