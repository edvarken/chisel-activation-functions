...
/**
  ...
  */
class geluUsingLUT(val intBits: Int = 2, val fracBits: Int = 4) extends Module {
    ...
    val lut = Module(new geluLUT(intBits, fracBits)) // LUT for the values between -4 and 4 (or -8 and 8)
...
/**
 * Generate Verilog sources and save it in generated/geluUsingLUT.v
