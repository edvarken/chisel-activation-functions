...
def printBF16AndFPValues(values):
    for i in range(len(values)):
        ...
        function_bf16_float = struct.unpack('>f', struct.pack('>I', function_bf16_int))[0]
        # Print the values
        print(f"({x0:.6f}, {i + 1}, {value_bf16_bits}, b{value_FP3intfrac9}, {function_bf16_float:.6f})")
...
if __name__ == "__main__":
    ...
    # printBF16AndFPValues(mirrored_y_intercepts)
    # print("\n=========================================================\n")
    ####### check if adding 3 brkpts at the end gives good results, no it does not
    ...
    breakpointsMoreEqualXsegmentsSILU = createBreakpoints(4, 6, (6-(4))/8, "silu-uniform-x") # 9 brkpts
    slopes, y_intercepts, mirrored_y_intercepts = calculateSlopesAndYIntercepts(breakpointsMoreEqualXsegmentsSILU)
    # print("\n====================== SiLU ========================\n")
    # print(slopes)
    # printBF16AndFPValues(breakpointsMoreEqualXsegmentsSILU)
    # print("\nBF16 and FP values for the 8 segments: slopes:")
    # printBF16AndFPValues(slopes)
    # print("\nBF16 and FP values for the 8 segments: y-intercepts:")
    # printBF16AndFPValues(y_intercepts)
    # print("\n====================================================\n")

    breakpointsAroundZeroEqualXsegmentsSILU = createBreakpoints(-1, 2, (2-(-1))/12, "silu-uniform-x") # 3 brkpts
    slopes, y_intercepts, mirrored_y_intercepts = calculateSlopesAndYIntercepts(breakpointsAroundZeroEqualXsegmentsSILU)
    print("\n====================== SiLU ========================\n")
    print(slopes)
    printBF16AndFPValues(breakpointsAroundZeroEqualXsegmentsSILU)
    print("\nBF16 and FP values for the 8 segments: slopes:")
    printBF16AndFPValues(slopes)
    print("\nBF16 and FP values for the 8 segments: y-intercepts:")
    printBF16AndFPValues(y_intercepts)
